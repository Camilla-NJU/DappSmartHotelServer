...
//                            List response = cal.bidvalue(orderResponse.getBids());
                            if (response == null) {
                                if (cal.getGoodBid().size() == 1) {         //just one good bid; and put the result in the LinkedBlockingQueue
                                    ...
                                    multiAgent.behavior.message.Negotiation neg = new multiAgent.behavior.message.Negotiation(myAgent,allbids,"",true);
...
//                                    agent.doDelete();      //clear the agent
                                ...
                                myAgent.addBehaviour(new multiAgent.behavior.message.Negotiation(myAgent, response, order.getId(),false));
                            }
...
//                            myAgent.addBehaviour(new Negotiation(myAgent, orderResponse.getBids()));
                ...
                try {
                    ContentElement ce = myAgent.getContentManager().extractContent(msg);
                    Action act = (Action) ce;
                    if (act.getAction() instanceof multiAgent.ontology.Negotiation) {
                        multiAgent.ontology.Negotiation negotiation = (multiAgent.ontology.Negotiation) act.getAction();
                    ...
                    if (currentResponse == responseNum) {
                        ...
                        if (lowerPriceNum == 0) {
                            ...
                            java.util.List<BidInfo> resultBids = this.creatBidInfo(cal.getGoodBid());
                            agent.putResult(resultBids);
                            Negotiation neg = new multiAgent.behavior.message.Negotiation(myAgent,allbids,"",true);
                            myAgent.addBehaviour(neg);
                            ...
                            if (results == null) {
                                ...
                                agent.putResult(resultBids);
                                multiAgent.behavior.message.Negotiation neg = new multiAgent.behavior.message.Negotiation(myAgent,allbids,order.getId(),true);
                                myAgent.addBehaviour(neg);
//                                agent.doDelete();         //clear the agent
                            } else {
                                responseNum = results.size();
                                myAgent.addBehaviour(new multiAgent.behavior.message.Negotiation(myAgent, results, order.getId(),false));
                                bids.clear();
                            }
                        }
                    }
